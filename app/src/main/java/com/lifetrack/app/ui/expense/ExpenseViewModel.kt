...
class ExpenseViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private val _classifyStatus = MutableStateFlow<String?>(null)
    val classifyStatus: StateFlow<String?> = _classifyStatus

    /**
     * Runs the merchant keyword classifier over every still-uncategorized transaction.
     * Safe to call repeatedly -- it only ever fills in rows that are STILL null, so it can't
     * override anything the user (or a learned rule) already set.
     */
    fun runMerchantClassification() = viewModelScope.launch {
        val result = repo.classifyUncategorized()
        _classifyStatus.value = if (result.totalTagged == 0) {
    ...
    /** Reverts every keyword guess back to Uncategorized. Manual and rule-based ones are untouched. */
    fun undoMerchantClassification() = viewModelScope.launch {
        val n = repo.undoKeywordAutoTags()
        _classifyStatus.value = if (n == 0) "Nothing to undo." else "Reverted $n auto-tag(s)."
    }

    fun clearClassifyStatus() { _classifyStatus.value = null }
